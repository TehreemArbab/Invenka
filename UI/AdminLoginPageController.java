...
public class AdminLoginPageController {
    ...
    void LoginAdmin(ActionEvent event) throws IOException {
    		//Do Actual Work
    		//Incase It Doesn't Show; incaseerror label mein info
    	...
    	if(idtext.getText().isEmpty() || passwordtext.getText().isEmpty()) {
			...
			boolean answer = m.getBLController().LoginAdmin(id, password);
				if (answer == true) {
			    	m.changeScene("AdminAllFunctionsPage.fxml");
				}
				else if (answer == false) {
					incaseerror.setText("Admin Does Not Match Records");
				}    	
    	}
    }
}